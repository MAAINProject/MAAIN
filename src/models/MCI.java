...
public class MCI {
	...
	/*
	 * public void setC(int n,float Mat[][]) { int i,j;
	 * 
	 * for(i=0;i<n;i++) { for(j=0;j<n;j++) {
	 * 
	 * if(Mat[i][j]!=0) C.add(Mat[i][j]); } } }
	 */
	...
	/*
	 * public void setL(int n , float Mat[][]) { L.add(0); int count; for(int
	 * i=0;i<n;i++) { count=0; for(int j=0;j<n;j++) { if(Mat[i][j]!=0) count+=1;
	 * } if(count==0) { L.add(L.get(L.size()-1)); } else {
